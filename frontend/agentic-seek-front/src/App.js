...
function App() {
    ...
    // Use environment variable for backend URL, fallback to Docker gateway IP or localhost for development
    const BACKEND_URL = process.env.REACT_APP_BACKEND_URL || 'http://172.18.0.1:8000' || 'http://localhost:8000';
        ...
        <div className="app">
            ...
            <div className="system-status-panel">
                <div className="status-item">
                    <label>Status:</label>
                    <span className="status-value">{status || 'Ready'}</span>
                </div>
                <div className="status-item">
                    <label>Backend:</label>
                    <span className={`status-value ${isOnline ? 'healthy' : 'down'}`}>
                        {isOnline ? '🟢 Healthy' : '🔴 Down'}
                    </span>
                </div>
                <div className="status-item">
                    <label>Processing:</label>
                    <span className={`status-value ${isLoading ? 'active' : 'idle'}`}>
                        {isLoading ? '⏳ Active' : '✓ Idle'}
                    </span>
                </div>
                {responseData?.done !== undefined && (
                    <div className="status-item">
                        <label>Task Complete:</label>
                        <span className={`status-value ${responseData.done ? 'done' : 'processing'}`}>
                            {responseData.done ? '✅ Yes' : '⌛ In Progress'}
                        </span>
                    </div>
                )}
            </div>
